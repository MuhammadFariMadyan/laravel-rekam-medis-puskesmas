...
0cm'><span style='font-size:1.0pt;font-family:"Times New Roman","serif"'>&nbsp;</span></p>
  ...
  <h7>&nbsp;</h7>
                    @if(isset($dataLapRedis))  
                      @foreach($dataLapRedis as $itemRedis)               
                       <center>
                          <u><font size="14">DATA REKAM MEDIS PASIEN RAWAT JALAN<br></u>
                          </font>  
                          <h7>&nbsp;</h7>        
                       </center>                          
                            <table >
                                <tbody>
                                  ...
                                  <td><span class="badge bg-white">: {{$itemRedis->no_redis}}  </span></td>
                                </tr>
                                <tr>                                  
                                  <td>Nama Pasien </td>                                  
                                  <td><span class="badge bg-white">:   {{$itemRedis->nama_pasien}}</span></td>
                                </tr>
                                <tr>
                                  <td>Tanggal Lahir </td>                                  
                                  <td><span class="badge bg-light-white">: {{date('d - m - Y', strtotime($itemRedis->tgl_lahir))}}</span></td>
                                </tr>
                                <tr>
                                  <td>Alamat </td>                                  
                                  <td><span class="badge bg-light-white">: {{$itemRedis->alamat}}</span></td>
                                </tr>                                                                          
                              </tbody>
                            </table>                                                                
                       @endforeach     
                    ...
                    @if (! empty($dataLapRedis))
                     <?php $i=1;foreach ($dataLapRedis as $itemLaporan):  ?>
                      <tr>                        
                        ...
                        <td align="center">{{$itemLaporan->Anamnesia}}</td>
                        ...
                        <td align="center">{{$itemLaporan->rcn_tndk_lnjt}}</td>
                        <td align="center">{{$itemLaporan->cat_kprawatan}}</td>
                        <td align="center">{{$itemLaporan->layanan_lain}}</td>
                        <td align="center">{{$itemLaporan->paraf_vld_dktr}}</td>                        
